...
static ssize_t subdir_current_usage_show(struct kobject *kobj,
					 struct kobj_attribute *attr, char *buf)
{
	tmp = getDemodevKobjProcess(kobj);
	if (tmp)
		return sprintf(buf, "%d\n", tmp->current_buf_usage);
	return sprintf(buf, "%d\n", -1);
}

static struct kobj_attribute subdir_current_usage_attribute =
__ATTR(current_usage, 0444, subdir_current_usage_show, NULL);

static ssize_t subdir_total_bytes_read_show(struct kobject *kobj,
...
static struct kobj_attribute subdir_total_bytes_read_attribute =
__ATTR(total_bytes_read, 0444, subdir_total_bytes_read_show, NULL);

static ssize_t subdir_total_bytes_written_show(struct kobject *kobj,
					       struct kobj_attribute *attr,
					       char *buf)
{
	tmp = getDemodevKobjProcess(kobj);
...
static struct kobj_attribute subdir_total_bytes_written_attribute =
__ATTR(total_bytes_written, 0444, subdir_total_bytes_written_show, NULL);
...
static struct kobj_list *getDemodevKobjProcessByName(void)
{
	cpid = current->pid;
	snprintf(pid, 10, "%d", (int)cpid);
	list_for_each_safe(pos, q, &demodev_kobj_list.list) {
			 ...
			 char *buffer, size_t length, loff_t * offset)
{
	...
	*offset += length;
	...
	tmp = getDemodevKobjProcessByName();
	tmp->current_buf_usage = 0;
...
static struct kobj_attribute demodev_buf_size_attribute =
__ATTR(buffer_size, 0644, demodev_buf_size_show, demodev_buf_size_set);
...
static struct kobj_attribute demodev_usage_count_attribute =
__ATTR(usage_count, 0444, demodev_usage_count_show, NULL);

static ssize_t demodev_current_usage_show(struct kobject *kobj,
					  struct kobj_attribute *attr,
					  char *buf)
{
	return sprintf(buf, "%d\n", current_usage);
...
static struct kobj_attribute demodev_current_usage_attribute =
__ATTR(current_usage, 0444, demodev_current_usage_show, NULL);

static ssize_t demodev_total_bytes_read_show(struct kobject *kobj,
					     struct kobj_attribute *attr,
					     char *buf)
{
	return sprintf(buf, "%d\n", total_bytes_read);
...
static struct kobj_attribute demodev_total_bytes_read_attribute =
__ATTR(total_bytes_read, 0444, demodev_total_bytes_read_show, NULL);

static ssize_t demodev_total_bytes_written_show(struct kobject *kobj,
						struct kobj_attribute *attr,
						char *buf)
{
	return sprintf(buf, "%d\n", total_bytes_written);
...
static struct kobj_attribute demodev_total_bytes_written_attribute =
__ATTR(total_bytes_written, 0444, demodev_total_bytes_written_show, NULL);
